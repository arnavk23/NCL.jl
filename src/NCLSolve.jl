...
"""Return the name of the subproblem solver `sub`."""
name(sub::AbstractNCLSubSolver) = sub.name

"""Return the `GenericExecutionStats` stored in the subproblem solver `sub`."""
stats(sub::AbstractNCLSubSolver) = sub.stats

"""Return the current initial barrier parameter `mu_init` of the subproblem solver `sub`."""
mu_init(sub::AbstractNCLSubSolver) = sub.mu_init

"""Return the elapsed time from the last subproblem solve."""
elapsed_time(sub::AbstractNCLSubSolver) = sub.stats.elapsed_time

"""Return `true` if the execution stats indicate failure (status is not `:first_order`)."""
...
Solve a constrained nonlinear optimization problem using Algorithm NCL.

If an `AbstractNLPModel` is provided, it is automatically wrapped in an [`NCLModel`](@ref).
Unconstrained or bound-constrained problems will raise an error; pass those directly to an appropriate solver instead.

### Algorithm overview

At each outer iteration, NCL solves an augmented subproblem of the form

    minimize   f(x) + y'r + (ρ/2) ‖r‖²

using an interior-point subproblem solver. After each subproblem solve:

- If the residual norm `‖r‖∞` is below the current primal feasibility tolerance `η`,
  the Lagrange multipliers are updated (`y += ρ r`) and the tolerances `η` and `ω` are tightened.
- Otherwise, the penalty parameter `ρ` is increased by a factor of `τ_ρ = 10`.

The algorithm terminates when:
1. `‖r‖∞ ≤ feas_tol` and the dual feasibility residual ≤ `opt_tol` (success);
2. the maximum number of outer iterations `max_iter_NCL` is reached;
3. the penalty parameter reaches `ρ_max = 1e12` and the problem is declared infeasible.

### Arguments

* `ncl::NCLModel` — the NCL subproblem model (or any `AbstractNLPModel` with nonlinear constraints).
...
* `subsolver::AbstractNCLSubSolver` — subproblem solver (default: `IpoptNCLSubSolver(ncl)`; requires `NLPModelsIpopt.jl`).
* `verbose::Bool = true` — whether to display iteration log.

Additional keyword arguments are forwarded to the subproblem solver.

### Return value

A `GenericExecutionStats` with:
- `status`: `:first_order` (converged), `:infeasible`, or `:max_iter`.
- `solution`: the optimal `x` (excluding residual variables).
- `multipliers`: the final Lagrange multiplier estimates `y`.
- `solver_specific[:residuals]`: the final residual vector `r`.
...
f(x) = x[1] + x[2]
nlp = ADNLPModel(f, [0.5, 0.5], [0.0, 0.0], [1.0, 1.0],
  x -> [x[1]^2 + x[2], x[1] * x[2]], [1.0, -Inf], [Inf, 0.5])

stats = NCLSolve(nlp)  # wraps in NCLModel automatically
